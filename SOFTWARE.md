...
När skriviprompten når skivytans slut genomför vi ett "jump". Istället för att skrolla i vanlig bemärkelse så hoppar texten upp så att den bara fyller skärmens första fem rader. Skrivprompten fortsätter sitt jobb på rad sex.
...
Här är några mindre detaljer i specifikationen som du bör överväga att förtydliga innan vi börjar koda:Beslutet om Glyph Caching: Under "Prio 1" nämner du alternativen, men du bör uttryckligen skriva in att "Alternativ 1 (Uppackad buffert)" är det vi valt för implementation. Att låta 1 pixel ta upp 1 hel byte i RAM minimerar processorns overhead helt inför SPI-överföringen och garanterar lägsta möjliga latens.  Kontinuerlig lagring vs. F2 (Spara): I projektkraven anger du att "All text ska skrivas kontinuerligt till SD-kortet", men under funktionsknappar är F2 definierad som att tvinga en manuell skrivning. Det är bra att klargöra hur dessa interagerar i praktiken. (Till exempel: Motorn dumpar kontinuerligt tecken till en dold temp-fil för säkerhet, medan F2 städar upp och sparar ner en ren, färdig .txt-fil).  Urklippets (Clipboard) livslängd: När du kopierar text med Ctrl + C, ska den datan enbart existera i RAM (och därmed försvinna om du bryter strömmen), eller ska vi spara urklippet till en tillfällig fil på SD-kortet för att överleva omstarter?  Föreslagen prioriteringsordning för C-programmeringenEftersom vi bygger direkt mot hårdvaran och prioriterar prestanda, måste vi utveckla inifrån och ut. Här är den mest logiska ordningen:  Hårdvarulagret (SPI & Skärm): Börja med att initiera IT8951 via BCM2835-biblioteket med dina exakta panelinställningar (VCOM 2140, 1448x1072). Allokera minnet, ladda det uppackade typsnittet i RAM och bekräfta att du kan rita ett enda knivskarpt tecken (32x64 pixlar) på skärmen.  Inmatningsloopen (A2-läget): Koppla in Linux evdev för att läsa av ditt mekaniska tangentbord. Bygg sedan kärnloopen som reagerar på knapptryck och skickar bitmappar över SPI i det asynkrona A2-läget. Målet här är enbart att verifiera att vi klarar 6.7 tecken per sekund utan lagg.  Skrivmotorn & Layout: Hårdkoda marginalerna (44px topp, 68px sidor/botten) och implementera logiken för 41 tecken per rad och radbrytning (word wrap). Bygg in "jump"-funktionen så att prompten landar på sjätte raden när botten nås.  Minnesstruktur & Redigering: Hantera den interna minnesrepresentationen av texten. Implementera piltangenter, Shift-markering (som ritas med understrykning i DU-läget) och logiken för tyst städning vid inaktivitet.  Filhantering (F2, F3, F4): Integrera C-funktioner för att läsa/skriva textfiler till SD-kortet, samt tillståndsmaskinen (State Machine) som låter dig cykla mellan dokument och hålla koll på senast aktiva fil.  Nätverk & Synk (F5): Som sista steg kopplar du tangenttryckningen av F5 (och Shift+F5) till att trigga det systemanrop i Linux som kör dina färdiga skript för rfkill, Tailscale och okrypterad rsync.  När det gäller steg 4 (Minnesstrukturen), lutar du åt att bygga en klassisk "gap buffer" i RAM för att extremt snabbt kunna skjuta in text vid redigering, eller föredrar du att vi bygger en enklare liststruktur över textraderna?
